...
struct VelocityTracker {
    ...
    mutating func add(_ point: CGPoint, at time: TimeInterval) {
        ...
        samples.removeAll { time - $0.time > Config.velocityWindow }
    }
    ...
    func velocity(at now: TimeInterval) -> CGVector {
        let recent = samples.filter { now - $0.time <= Config.velocityWindow }
...
enum Fling {
    ...
    static func project(_ v: CGFloat, decelerationRate r: CGFloat = Config.flingDecelerationRate) -> CGFloat {
        (v / 1000) * r / (1 - r)
    }

    static func speed(_ v: CGVector) -> CGFloat { hypot(v.dx, v.dy) }

    static func isFling(_ v: CGVector) -> Bool { speed(v) >= Config.flingMinSpeed }
